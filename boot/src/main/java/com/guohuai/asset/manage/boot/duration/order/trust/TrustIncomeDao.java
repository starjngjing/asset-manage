...
public interface TrustIncomeDao extends JpaRepository<TrustIncomeEntity, String>, JpaSpecificationExecutor<TrustIncomeEntity> {
	...
	@Query(value = "update T_GAM_ASSETPOOL_TARGET_INCOME set state = '-1' where oid = ?1 and operator = ?2", nativeQuery = true)
	@Modifying
	public void updateOrder(String oid, String operator);
	
	@Query("from TrustIncomeEntity a where a.trustEntity.oid = ?1 and a.state <> '-1'")
