...
public class ProductService {
			...
			if (Product.DATE_TYPE_ManualInput.equals(form.getRaiseStartDateType())) {
				...
				if (!StringUtil.isEmpty(form.getRaisePeriod())) {
					...
					if (!StringUtil.isEmpty(form.getInterestsFirstDate())) {
						...
						if (!StringUtil.isEmpty(form.getDurationPeriod())) {
							pb.durationPeriodEndDate(new Date(
									DateUtil.addDay(raiseStartDate, Integer.valueOf(form.getRaisePeriod()) + Integer.valueOf(form.getInterestsFirstDate()) + Integer.valueOf(form.getDurationPeriod()))
											.getTime()));// 存续期结束时间
							if (!StringUtil.isEmpty(form.getAccrualDate())) {
								java.util.Date repayDate = DateUtil.addDay(raiseStartDate, Integer.valueOf(form.getRaisePeriod()) + Integer.valueOf(form.getInterestsFirstDate())
										+ Integer.valueOf(form.getDurationPeriod()) + Integer.valueOf(form.getAccrualDate()));
	...
	public BaseResp saveCurrent(SaveCurrentProductForm form, String operator) throws ParseException {
		...
		}
		{
			...
			pb.purchaseConfirmDaysType(form.getPurchaseConfirmDateType()).redeemConfirmDaysType(form.getRedeemConfirmDateType());
			// 产品成立时间（存续期开始时间）
		...
		if (Product.DATE_TYPE_FirstRackTime.equals(form.getRaiseStartDateType())) {
			...
			if (!StringUtil.isEmpty(form.getRaisePeriod())) {
				...
				if (!StringUtil.isEmpty(form.getInterestsFirstDate())) {
					...
					if (!StringUtil.isEmpty(form.getDurationPeriod())) {
						product.setDurationPeriodEndDate(new Date(
								DateUtil.addDay(raiseStatrtDate, Integer.valueOf(form.getRaisePeriod()) + Integer.valueOf(form.getInterestsFirstDate()) + Integer.valueOf(form.getDurationPeriod()))
										.getTime()));// 存续期结束时间
						if (!StringUtil.isEmpty(form.getAccrualDate())) {
							// 到期最晚还本付息日 指存续期结束后的还本付息最迟发生在存续期后的第X个自然日的23:59:59为止
							java.util.Date repayDate = DateUtil.addDay(raiseStatrtDate, Integer.valueOf(form.getRaisePeriod()) + Integer.valueOf(form.getInterestsFirstDate())
									+ Integer.valueOf(form.getDurationPeriod()) + Integer.valueOf(form.getAccrualDate()));
	...
	public ProductLog getProductLog(final String oid, final String auditType, final String auditState) {
		...
		Specification<ProductLog> spec = new Specification<ProductLog>() {
			...
			public Predicate toPredicate(Root<ProductLog> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				return cb.and(cb.equal(root.get("product").get("oid").as(String.class), oid), cb.equal(root.get("auditType").as(String.class), auditType),
